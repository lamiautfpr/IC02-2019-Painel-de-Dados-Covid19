...
def insert(session):
    ...
    complements = ['', '_-_copy','__0', '_', '%20', '_atualizado', '_1', '_0']
    prefixes = ['', '_']
    texto = 'informe_epidemiologico'
    base_url = 'https://www.saude.pr.gov.br/sites/default/arquivos_restritos/files/documento/{}/{}{}_{}{}.pdf'
                ...
                if start_date != datetime(2020, 8, 12, 14, 0, 0).date():
                    for pfx in prefixes:
                        for com in complements:
                            ...
                            try:
                                response = requests.get(url, timeout=10, headers=headers) # url com texto em lowercase
                                print(url)
                            ...
                            try:
                                response = requests.get(url,  timeout=10, headers=headers) # url com texto em uppercase
                                print(url)
                                if response.ok: # se True
                                    data_check = True
                                    break # quebra loop complements
