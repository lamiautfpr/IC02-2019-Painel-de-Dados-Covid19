...
def cleaner(dataset, word):
    dataset.rename(columns={'Value': '{}'.format(
        word.capitalize())}, inplace=True)
    dataset = dataset[1:]
    return dataset
...
def catcher():
    ...
    for word in data:
        ...
        if word == "confirmed":
            temp_dataset = pd.read_csv(url, encoding='ISO-8859-1',
...
def insert(session):
    print("Inserindo get_hdx_mundo")

    db_format = table_class.Hdx_mundo()
    dataset = catcher()

    dataset.to_sql('HDX_base_mundo', con=session.get_bind(),
                   index_label='id', if_exists='replace', method='multi',
                   chunksize=50000, dtype=db_format)
    return print("hdx_mundo inserido com sucesso!")
