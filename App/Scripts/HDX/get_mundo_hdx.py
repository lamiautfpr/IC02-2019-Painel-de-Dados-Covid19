...
def catcher():
    ...
    for word in data:
        ...
        if word == "confirmed":
            temp_dataset = pd.read_csv(url, sep=',|;', encoding='ISO-8859-1', engine='python', error_bad_lines=False, usecols=["Country/Region", "Date", "Value"])    
        else:
            temp_dataset = pd.read_csv(url, sep=',|;', encoding='ISO-8859-1', engine='python', error_bad_lines=False, usecols=["Value"])  
        temp_dataset.rename(columns={'Value': '{}'.format(word.capitalize())}, inplace=True)

        dataset = pd.concat([dataset, temp_dataset], axis=1)
    
    cleaner(dataset)

    return print(dataset.head())
